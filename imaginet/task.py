...
def representation(model, sents, batch_size=128, task=None):
...
def states(model, sents, batch_size=128, task=None):
    ...
    """
    return [ r[:,-1,:] for r in pile(model, sents, batch_size=128, task=task) ]

def pile(model, sents, batch_size=128, task=None, include_end=False):
...
def encode_sentences(model, sents, batch_size=128, task=None):
